...
def ks_test(simulated_df, n_group1):
    ...
    for i in range(0, len(simulated_df.index)):
        gr1 = simulated_df.iloc[i, 0:n_group1]
        gr2 = simulated_df.iloc[i, (n_group1+1):len(simulated_df.columns)]
        k_test_statistic, k_test_pvalue = scipy.stats.ks_2samp(gr1, gr2)
...
def limma_test(simulated_df, n_group1, n_group2):
    ...
    #print(simulated_df, design_matrix)
    #print(simulated_df.shape, design_matrix.shape)
...
def logit_transformation(beta_matrix):
    ...
    return m_matrix
    #https://www.frontiersin.org/articles/10.3389/fnins.2021.701017/full#B12
    #list_of_cpgs = [row for row in range(0, len(beta_matrix.index))]
    #for i in list_of_cpgs:
    #    beta_matrix.iloc[i,:] = sesame.BetaValueToMValue(beta_matrix.iloc[i,:])
        #for j in range(0, len(beta_matrix.columns)):
        #    if beta_matrix.iloc[i,j] >= float(1):
        #        beta_matrix.iloc[i, j] = float(0.99)
        #    elif beta_matrix.iloc[i,j] <= float(0):
        #        beta_matrix.iloc[i, j] = 0.00000000000000001
    #m_matrix = log2(np.array(beta_matrix)/(1-(np.array(beta_matrix))))
    #print(beta_matrix)
    #return beta_matrix
...
def run_all_test(all_zips, workflow_num, beta_or_M_string):
    ...
    test_dict = {"T-test": t_test_output, "KS-test": ks_test_output,"T-limma": limma_test_output}
...
def multitest_p_adjust(all_test_df, p_adjust_method_string):
    ...
    store_p_adjust.columns = ["T-test", "KS-test", "limma"]
...
def create_predicted_vector(p_val_vector, p_cut):
    bool_signif_p_val = [row for row in range(0, len(p_val_vector.index))]
    for p_value in range(0, len(p_val_vector.index)):
        if p_val_vector.iloc[p_value] < p_cut:
            bool_signif_p_val[p_value] = False
        else:
            bool_signif_p_val[p_value] = True
...
def create_expected_val_vector(p_val_df, all_runs_zip_num,workflow_num):
    ...
    boolean_truly_modifed = np.isin(indices_p_val_vector, indices_truly_different, invert=True)
...
def create_confusion_matrix(all_test_df, all_runs_zip_num, workflow_num, p_cut):
    ...
    for i in range(0, len(all_test_df.columns)):
        predicted = create_predicted_vector(all_test_df.iloc[:,i], p_cut)
        workflows_confusion_matrix.append(caret.confusionMatrix(data=FactorVector(robjects.BoolVector(predicted)), reference=FactorVector(robjects.BoolVector(expected))))
...
def calc_empirical_marg_power(workflows_confusion_matrix):
    ...
    df_all_test['Test'] = ['T_test', 'KS_test', 'Limma_test']
...
def plot_line(data_df, y_parameter_string, varied_parameter_string, p_adjust_method_string, beta_or_M_string):
    ...
    #column_names = selected_parameters_df.columns
    #print("column_names1")
    #print(column_names)
    #column_names = sub(y_parameter, "y_parameter", column_names)
    #print("column_names2")
    #print(column_names)
    #column_names = sub(varied_parameter, "varied_parameter", column_names)
    #print("column_names3")
    #print(column_names)
    #selected_parameters_df.columns = column_names

    #for i in range(0, len((selected_parameters_df[varied_parameter]).unique())):
        #print(i)
        #selected_parameters = selected_parameters_df[selected_parameters_df[varied_parameter] == selected_parameters_df.iloc[i,varied_parameter].unique(),:]
    line_plot_fig = sns.lineplot(data=selected_parameters_df, x=y_parameter, y=selected_parameters_df['Power'], hue=selected_parameters_df['Test'])
    plt.title("Sample Vs Power (Effect=0.05)")
    plt.xlabel('Sample')
    plt.ylabel('Power')
    plt.ylim(0.9,1)
    #plt.show()
    varied_param_i = selected_parameters_df[varied_parameter].unique()
    fig = line_plot_fig.get_figure()
    fig.savefig(str("LinePlot_") + str("_vs_Power_") + varied_parameter + str("_step_") +
                    str(varied_param_i.values) + "_" + p_adjust_method_string + "_" + beta_or_M_string + ".png", dpi=100)

def heat_map(data_df, x_parameter_string, y_parameter_string, test_string, p_adjust_method_string, beta_or_M_string):
    #selected_parameters_df = data_df[:, data_df.columns in ["Test", "Power", x_parameter_string, y_parameter_string]]
    y_parameter = y_parameter_string
    x_parameter = x_parameter_string

    #column_names = selected_parameters_df.columns
    #column_names = sub(y_parameter, "y_parameter", column_names)
    #column_names = sub(x_parameter, "x_parameter", column_names)
    #selected_parameters_df.columns = column_names

    #selected_parameters_df["Power"] = round(selected_parameters_df["Power"], 2)
    selected_parameters_df = data_df.loc[data_df.loc[:,"Test"] == test_string,:]
    #result = pd.pivot(data=selected_parameters_df, index=y_parameter, columns=x_parameter, values=selected_parameters_df["Power"])
    #result = selected_parameters_df.pivot("n_samples", "effect_size", "Power")

    fig, ax = plt.subplots(figsize=(10, 10))
    plt.title("HeatMap of Num Sammples by Effect size")
    print(selected_parameters_df)
    heat_map_plot = sns.heatmap(selected_parameters_df, fmt="", cmap='RdYlGn', annot=True,linewidths=0.30, ax=ax)
    plt.xlabel(x_parameter)
    plt.ylabel('Sample')
    plt.show()
    fig = heat_map_plot.get_figure()
    fig.savefig(str("HeatMap_") + y_parameter + "_vs_" +x_parameter+ "_colorfill_Power"+test_string+"_"+p_adjust_method_string+"_"+beta_or_M_string + ".png", dpi=100)
...
def PowerCalc(num_zips, p_adjust_method_string, beta_or_M_string, y_parameter_string, x_parameter_string, color_string, test_vector_string,p_cut):
    ...
    all_test_all_zips.to_csv(os.getcwd()+dirname+"PowerCalc_"+beta_or_M_string+"_"+p_adjust_method_string+'.csv', sep=" ", index=0,header=True)

    plot_line(all_test_all_zips, y_parameter_string, x_parameter_string,p_adjust_method_string,beta_or_M_string)

    #for test in range(0, len(test_vector_string)):
    #    print(test)
...
num_zips = 8 # number of simulated data environmental workflows
...
color_string = "Power"
test_vector_string = ["T_test","Limma_test", "KS_test"]
p_cut = 0.05 #0.01, 0.1
